 ...
 * 
 * 특정 사용자가 참여한 채팅방 목록, 각 채팅방의 최근 메세지, 관련된 정보 등 조회목적
 * */
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인한 사람이 오픈멤버일때는 참가자 타겟번호)
		R.OPEN_MEMBER	-- NULL인 경우 		  (내가 오픈멤버가 아닐 때는 OPEN_MEMBER가 타켓번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
...
SELECT TO_CHAR(ENROLL_DATE, 'YY.MM.DD')
FROM "MEMBER";
...
FROM "MEMBER"
WHERE AUTHORITY = 1
AND ENROLL_DATE >= CURRENT_DATE - INTERVAL '7' DAY;