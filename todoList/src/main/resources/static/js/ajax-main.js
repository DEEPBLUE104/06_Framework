...
-> 비동기 작업이 완료되었을 때 실행할 콜백함수를 지정하고,
  ...
  - Fulfilled (이헹됨) : 비동기 작업이 성공적으로 완료된 상태
...
function getTotalCount() { // 함수 정의
  ...
  .then( response => { // 서버에서 응답을 받으면,
    ...
    // response.text() : 응답 테이터를 문자열/숫자 형태로 변환한 결과를 가지는
    //                   Promise 객체 반환
...
addBtn.addEventListener("click", () => {
  ...
  // JS <-> Java
  // JSON (Java Script Object Notation) : 데이터를 표현하는 문법
  // {"name" : "홍길동"
  // "age" : "20"
  ...
  fetch("/ajax/add", {
    ...
    body : JSON.stringify(param) // param이라는 JS 객체 > JSON(string)으로 변환
  })
...
const selectTodoList = () => {
  ...
  .then(todoList => {
    ...
    // 만약에 resp.text() 사용했다면 문자열(JSON이 그대로 노출)
    ...
    for(let todo of todoList) { // 향상된 for문 (of == :)
      // tr 태그 생성
      ...
      const arr = ['todoNo', 'todoTitle', 'complete', 'regDate'];
        ...
        for(let key of arr) {
          ...
          if(key === 'todoTitle'){
            ...
            // a태그 클릭시 페이지 이동 막기 (비동기 요청 사용 위해)
...
changeComplete.addEventListener("click", () => {
  ...
  .then(result => {
    ...
    if(result > 0) { // 성공
      ...
      selectTodoList();
      // 서버 부하 줄이기 가능! => 코드가 복잡해서 오히려 비용 증가
      // 그냥 서버 요청 함수 호출로 해결
