...
const initMin = 4;  // 타이머의 초기값(분)
const initSec = 59; // 타이머의 초기값(초)
const initTime = "5:00";
...
memberEmail.addEventListener("input", (e) => {
  ...
  // 3) 입력된 이메일이 없을 경우
    ...
    // 영어, 숫자, 특수문자(. _ % + -) 를 허용
    // 이메일 형식으로 작성
    // 도메인 부분은 최소 2개의 문자로 끝나야 함(.com, .net, .ong, .kr)
...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  // 화면에서는 타이머 시작하기
  authKeyMessage.innerText = initTime;  // 5:00 세탕
  authKeyMessage.classList.remove("comfirm", "error");  // 검정 글씨색

  alert("인증 번호가 발송되었습니다.");

  // setInterval(콜백함수, 지연시간(ms))
  // - 지연시간(ms)만큼 시간이 지날 때마다 콜백함수 수행
...
// 매개변수 전달받은 숫자가 10 미만인 경우 (한자리) 앞에 0을 붙여서 반환
