...
if (updateInfo != null) {
  ...
  updateInfo.addEventListener("submit", async (e) => {
    ...
    // 기존 닉네임이 저장되어있는 요소에 value 값 얻어오기
    const currentNickname = document.querySelector("#memberNickname").value;
    ...
    if (currentNickname != memberNickname.value) {
      // 비동기 요청 (fetch() API 이용)
      // async / await 사용
      // async : 비동기 함수를 만들 때 사용되는 키워드
      // (== "이 함수내에는 시간이 걸리는 작업이 있다!")
      ...
      const resp = await fetch(
        "/member/checkNickname?memberNickname=" + memberNickname.value
      );
    ...
    /*
        ...
        e.preventDefault()가 비동기 함수 이후에 호출되면 이미 form이 
        submit 되어버린 다음일 수 있다
        -> 비동기 작업 중에 form의 기본 제출 동작 일어나는 가능성이 생김
        */
  });
}
...
if (searchAddress != null) {
  //화면 상에 id가 searchAddress인 요소가 존재하는 경우에만
