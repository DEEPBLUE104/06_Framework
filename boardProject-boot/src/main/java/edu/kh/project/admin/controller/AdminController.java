...
@SessionAttributes({ "loginMember" }) // 세션에 등록하는 방법
public class AdminController {
	...
	@GetMapping("newMemberInWeekData")
	public ResponseEntity<Object> newMemberInWeekData(){
		try {
		
			List<Member> memberList = service.newMemberInWeekData();
	...
	public ResponseEntity<Object> selectWithdrawnMemberList() {
		// 성공식 List<Member> 반환, 실패 시 String 반환 -> 둘다 받아줄 수 있는 Object 사용
		try {
	...
	public ResponseEntity<Object> selectDeleteBoardList() {
		// 성공식 List<Board> 반환, 실패 시 String 반환 -> 둘다 받아줄 수 있는 Object 사용
		try {
