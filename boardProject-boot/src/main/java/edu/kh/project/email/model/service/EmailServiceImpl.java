...
public class EmailServiceImpl implements EmailService{
	...
	private String loadHtml(String authKey, String htmlName) {
		// Context : 타임리프에서 제공하는 HTML 템플릿에 데이터를 전달하기 위해
		//		 	 사용하는 클래스
	...
	@Transactional(rollbackFor = Exception.class) // 메소드 레벨에서도 이용 가능(해당 메서드에서만 트랜젝션 COMMIT/ROLLBACK)
	private boolean storeAuthKey(Map<String, String> map) {
		...
		// 2. UPDATE 수행 시 0이 반환됨 == UPDATE 안 됨 == INSERT된 내용이 없었다,
		//	 		INSERT 수행하면 된다.
		if(result == 0) {
