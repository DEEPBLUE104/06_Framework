...
VALUES(1, 1995); -- 1번 회원이 1995번 게시글에 좋아요를 누름
...
-- 특정 게시판(BOARD_CODE)에 삭제되지 않는 게시글 목록 조회
-- 단, 최신글이 제일 위에 존재하도록 조회
-- 작성일 : 몇 초/ 몇 분/ 몇 시간 전 조회, 하루가 넘어가면 YYYY-MM-DD 형식 조회

-- 게시글 번호/제목[댓글개수]/작성자닉네임/작성일/조회수/좋아요 개수
...
(
 ...
 UNION 
 SELECT NEXT_IMG_NO(), '경로2', '원본2', '변경2', 2, 1995 FROM DUAL
 UNION 
 SELECT NEXT_IMG_NO(), '경로3', '원본3', '변경3', 3, 1995 FROM DUAL
);
...
TO_CHAR(BOARD_WRITE_DATE, 'YYYY"년" MM"월" DD"일" HH24:MI:SS') BOARD_UPDATE_DATE,
...
DELETE FROM "REPORT" WHERE TARGET_MEMBER_NO = 1;

SELECT COUNT(*) AS "reportedByMe"
FROM REPORT
